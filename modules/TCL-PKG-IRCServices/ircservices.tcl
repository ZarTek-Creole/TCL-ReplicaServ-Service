...
namespace eval ::IRCServices {
	...
	if { [catch { package require ZCT ${PKG(need_zct)} } err] } {
		puts stderr "\[${PKG(name)} - erreur\] Nécessite le package ZCT ${PKG(need_zct)} (ou plus) pour fonctionner, Télécharger sur 'https://github.com/ZarTek-Creole/TCL-ZCT'.\nLe chargement du script a été annulé."
		return
		} else { namespace import -force ::ZCT::* }
		pkg load Tcl ${PKG(need_tcl)} ${PKG(name)}
		# logger (tcllib) optionnel
		if { [catch { pkg load logger ${PKG(need_logger)} ${PKG(name)} "puts" }] } {
			puts "\[${PKG(name)}\] Package logger absent (installez tcllib) — journalisation désactivée."
		}
	}

# Arrêt propre hors eggdrop (commande « die » absente en tclsh standalone)
proc ::IRCServices::fatal { msg {do_exit 1} } {
	catch { putlog "\[IRCServices\] $msg" }
	puts stderr "\[IRCServices\] $msg"
...
proc ::IRCServices::incrementChar { uid } {
	variable newCharArray
	set alphabet	"ABCDEFGHIJKLMNOPQRSTUVWXYZ"
	set uid			[string toupper $uid]
	if { [string length $uid] >= 9 } {
		set sid	[string range $uid 0 2]
		set id	[string range $uid 3 end]
	} else {
		set sid	""
		set id	$uid
	}
	if { $id eq "" } {
		return -code error "incrementChar: identifiant vide ($uid)"
	}
	set i	[expr {[string length $id] - 1}]
	while { $i >= 0 } {
		set c	[string index $id $i]
		set pos	[string first $c $alphabet]
		if { $pos < 0 } {
			return -code error "incrementChar: caractère invalide '$c' dans $uid"
		}
		if { $pos < [expr {[string length $alphabet] - 1}] } {
			set id	[string replace $id $i $i [string index $alphabet [expr {$pos + 1}]]]
			return "${sid}${id}"
		}
		set id	[string replace $id $i $i [string index $alphabet 0]]
		incr i -1
	}
	# Overflow : allonge d'un caractère (Z → AA, ZZ → AAA)
	return "${sid}A[string repeat A [string length $id]]"
}
...
# Create an IRC connection namespace and associated commands.
	...
	proc ::IRCServices::connection { args } {
		...
		proc UID_GET { user } {
				...
				set prefix	""
				if { [info exists serverinfo(ID)] && $serverinfo(ID) ne "" } {
					set prefix	$serverinfo(ID)
				} elseif { [info exists sid] && $sid ne "" } {
					set prefix	$sid
				}
				if { ${UID_LAST_INSERT} == "" } {
					set UID_LAST_INSERT		"${prefix}AAAAAA"
				} else {
					set UID_LAST_INSERT		[::IRCServices::incrementChar ${UID_LAST_INSERT}]
				}
				set UID_DB([string toupper ${user}])			${UID_LAST_INSERT}
				set UID_DB([string toupper ${UID_LAST_INSERT}])	${user}
		...
		proc UID_PREFIX_RANDOM { } {
			# 3e caractère du SID : A-Z / 2-9 (évite 0/1 et index hors bornes)
			set CHAR_LIST   {ABCDEFGHIJKLMNOPQRSTUVWXYZ23456789}
			set LENGTH      [string length ${CHAR_LIST}]
			set CHAR_WIN  	[expr {int(rand()*${LENGTH})}]
		...
		# Create the actual tcp connection.
			...
			proc cmd-connect { Server_HostName Server_Port Server_Password { Server_Protocol 1 } { Server_Name Extra-Cool.FR } { Server_ID "" } { Server_Description "Services for IRC Networks" } } {
				variable serverinfo
				variable sock
			...
			if { ${Server_ID} == "" } {
				...
				# sid utilisé par vusercreate / bot / mode
				variable sid
				set sid		$serverinfo(ID)
			...
			if { ${sock} eq "" } {
					if {
						[catch {
							set sock [{*}${socket_binary} ${Server_HostName} $serverinfo(port)]
						} err]
					} {
						::IRCServices::fatal "\[${PKG(name)} - Erreur\] Impossible de ce connecter sur ${Server_HostName}:${Server_Port}: ${err}"
					}
					...
					if { ${Server_Protocol} } {
						...
						send ":${serverinfo(ID)} SERVER ${Server_Name} 1 :${Server_Description}"
						send "EOS"
					} else {
						send "PASS ${Server_Password}"
						send "SERVER ${Server_Name} 1 :${Server_Description}"
						send "EOS"
					}
				}
				return 1
			...
			proc cmd-bot { args } {
				...
				# eventbind --
					...
					# Alias rétro-compat (ReplicaServ / ClaraServ)
					proc cmd-registerevent { event cmd } {
						cmd-eventbind ${event} ${cmd}
					}
			...
			if { ${DIE} != "" } { ::IRCServices::fatal "\[Error - IRCServices\] ${DIE}" }
		...
		# GetLine --
			...
			proc GetLine { } {
				...
				if {
					...
					::IRCServices::fatal "Error IRCServices: perte de la connexion au serveur"
					return
				}
								cmd-log debug "Recieved: ${line}"
				if { [string match -nocase "error*" [lindex [regexp -all -inline {\S+} ${line}] 0]] } { GetError ${line} }
				if { [set pos [string first " :" ${line}]] > -1 } {
					set headerText			[string range ${line} 0 [expr {${pos} - 1}]]
					set linedata(msg)		[string range ${line} [expr {${pos} + 2}] end]
				} else {
					set headerText			[string trim ${line}]
					set linedata(msg)		{}
				}
				set header					[regexp -all -inline {\S+} ${headerText}]
				if { [string match :* ${headerText}] } {
					set header				[lreplace ${header} 0 0 [string trimleft [lindex ${header} 0] :]]
				} else {
					set header				[linsert ${header} 0 {}]
				}
set linedata(rawline)		${line}
				...
				foreach t [namespace children] {
					...
					if { [info exists ${t}::dispatch(${linedata(action)})] } {
						set _cb [set ${t}::dispatch(${linedata(action)})]
					} elseif { [string match {[0-9]??} ${linedata(action)}] } {
						set _cb [set ${t}::dispatch(defaultnumeric)]
					} elseif { ${linedata(who)} eq "" } {
						set _cb [set ${t}::dispatch(defaultcmd)]
					} else {
						set _cb [set ${t}::dispatch(defaultevent)]
					}
					if { ${_cb} ne "" && ${_cb} ne "#" } {
						if { [catch { namespace eval [namespace current] ${_cb} } _err] } {
							cmd-log error "Bot callback ${linedata(action)} failed: ${_err}"
						}
					}
				}
				...
				if { ${_cb} ne "" && ${_cb} ne "#" } {
					if { [catch { namespace eval [namespace current] ${_cb} } _err] } {
						cmd-log error "Network callback ${linedata(action)} failed: ${_err}"
					}
				}
				if { [action] == "SLOG" } {
					# Unreal SLOG = journal serveur (ex: Lost server link…) — JAMAIS fatal
					cmd-log notice "SLOG: ${linedata(msg)}"
				}
				if { [action] == "UID" } {
